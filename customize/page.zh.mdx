...
> 其他区块，比如 `Hero` / `Footer` 等，自定义展示方式与 `Header` 类似，可以参考上述配置内容。

### 自定义区块

默认的着陆页配置，在网站首页，展示的区块包括：
...
<Step>
  在 `src/config/locale/messages/{locale}/landing.json`
  配置文件中，删除不需要的区块，或者添加自定义区块。
</Step>
...
<Step>
  在 `src/app/[locale]/(landing)/page.tsx` 文件中，调整 `showSections`
  数组，只包含需要显示的区块。
</Step>
